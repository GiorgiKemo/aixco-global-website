...
const copyByLanguage: Record<string, Copy> = {
  en: {
    ...
    body: "With your permission, Google Analytics through Google Tag Manager and optional AIXCO analytics use limited pseudonymous usage data—such as pages viewed, visit timing, interactions, device context, and approximate location—to understand site performance. We do not use analytics to identify you by name, build an advertising profile, or sell your information. Google Analytics receives measurement data when enabled. We never record form contents or passwords. Change your choice anytime in the footer.",
  ...
  de: {
    ...
    body: "Mit Ihrer Zustimmung verwenden Google Analytics über Google Tag Manager und optionale AIXCO-Analysen begrenzte pseudonyme Nutzungsdaten—zum Beispiel aufgerufene Seiten, Besuchszeiten, Interaktionen, Gerätekontext und ungefähre Standortdaten—um die Website zu verstehen. Wir identifizieren Sie damit nicht namentlich, erstellen kein Werbeprofil und verkaufen Ihre Informationen nicht. Google Analytics erhält Messdaten, wenn es aktiviert ist. Formularinhalte und Passwörter werden niemals erfasst. Ihre Auswahl können Sie jederzeit in der Fußzeile ändern.",
  ...
  pl: {
    ...
    body: "Za zgodą Google Analytics przez Google Tag Manager i opcjonalna analityka AIXCO używają ograniczonych, pseudonimowych danych—takich jak odwiedzane strony, czas wizyty, interakcje, kontekst urządzenia i przybliżona lokalizacja—aby rozumieć działanie strony. Nie identyfikujemy Cię za ich pomocą po imieniu, nie tworzymy profilu reklamowego i nie sprzedajemy informacji. Google Analytics otrzymuje dane pomiarowe po włączeniu. Nie rejestrujemy treści formularzy ani haseł. Wybór możesz zmienić w stopce.",
  ...
  sl: {
    ...
    body: "Z vašim dovoljenjem Google Analytics prek Google Tag Manager in izbirna analitika AIXCO uporabljata omejene psevdonimne podatke—na primer ogledane strani, čas obiska, interakcije, kontekst naprave in približno lokacijo—za razumevanje delovanja strani. Ne identificiramo vas po imenu, ne ustvarjamo oglaševalskega profila in ne prodajamo informacij. Google Analytics prejme merilne podatke, ko je omogočen. Vsebine obrazcev in gesel ne beležimo. Izbiro lahko kadar koli spremenite v nogi strani.",
  ...
  ru: {
    ...
    body: "С вашего разрешения Google Analytics через Google Tag Manager и необязательная аналитика AIXCO используют ограниченные псевдонимные данные—например, просмотренные страницы, время визита, взаимодействия, сведения об устройстве и примерное местоположение—чтобы понимать работу сайта. Мы не идентифицируем вас по имени, не создаём рекламный профиль и не продаём информацию. При включении Google Analytics получает данные измерений. Мы не записываем содержимое форм или пароли. Вы можете изменить выбор в нижней части сайта.",
