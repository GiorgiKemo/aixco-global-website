...
type TeamPortraitLoading = {
  isTeamInView: boolean;
  index: number;
};

export function getTeamPortraitLoading({ isTeamInView, index }: TeamPortraitLoading) {
  const shouldPrioritize = isTeamInView && index < 3;
              ...
              >
                <div className="relative aspect-[9/10] overflow-hidden bg-muted">
                  <Image
                    ...
                    alt={m.name}
                    unoptimized
                    loading={portraitLoading.loading}
                    fetchPriority={portraitLoading.fetchPriority}
                    decoding="async"
                    width={832}
                    height={1024}
