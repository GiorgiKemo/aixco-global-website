...
function signedCurrency(value: number, lang: string) {
...
function OutputCard({ label, value, accent = false }: { label: string; value: string; accent?: boolean }) {
  return (
    <article className={`min-h-[9.5rem] border border-[#161616]/12 p-5 sm:p-6 ${accent ? "bg-[#002147] text-white" : "bg-white text-[#161616]"}`}>
      <p className={`text-[0.62rem] font-semibold uppercase tracking-[0.18em] ${accent ? "text-[#E6C767]" : "text-[#161616]/70"}`}>{label}</p>
      <p className={`mt-7 font-display text-[clamp(1.8rem,3.6vw,3.2rem)] font-semibold leading-none tracking-[-0.05em] ${accent ? "text-white" : "text-[#161616]"}`}>{value}</p>
    </article>
  );
}
    ...
    <>
      ...
      <main id="main-content" className="bg-[#F3EDE1] text-[#161616]">
            ...
            <div className="mt-10 grid gap-5 lg:grid-cols-[0.84fr_1.16fr] lg:gap-8">
              ...
              <section className="grid content-start gap-px bg-[#161616]/12" aria-label={tx("Your scenario")}>
                <div className="grid gap-px bg-[#161616]/12 sm:grid-cols-3">
                  <OutputCard label={tx("Invested equity")} value={formatCurrency(calculation.investedEquity, lang)} />
                  <OutputCard label={tx("Monthly surplus")} value={signedCurrency(calculation.monthlySurplus, lang)} accent />
                  <OutputCard label={`${tx("Net worth after")} ${inputs.holdingYears} ${tx(inputs.holdingYears === 1 ? "year" : "years")}`} value={formatCurrency(calculation.holdingProjection.netWorth, lang)} />
                </div>
                <div className="bg-white p-6 sm:p-8 lg:p-10">
                  <div className="flex flex-col justify-between gap-4 border-b border-[#161616]/12 pb-5 sm:flex-row sm:items-end">
                    <div><p className="text-[0.62rem] font-semibold uppercase tracking-[0.18em] text-[#8B6A18]">{tx("At completion")}</p><h3 className="mt-3 font-display text-3xl font-medium tracking-[-0.05em]">{formatCurrency(calculation.completionValue, lang)}</h3></div>
                  ...
                  <div className="mt-2 grid gap-0 sm:grid-cols-2 sm:gap-x-10">
                    <DetailRow label={tx("Purchase price")} value={formatCurrency(calculation.listPrice, lang)} />
                    <DetailRow label={tx("Financing amount")} value={formatCurrency(calculation.loanAmount, lang)} />
                    <DetailRow label={tx("Net monthly rent")} value={formatCurrency(calculation.netMonthlyRent, lang)} strong />
                    <DetailRow label={tx("Monthly bank payment")} value={`− ${formatCurrency(calculation.monthlyBankPayment, lang)}`} />
                  </div>
                </div>
                <div className="bg-[#002147] p-6 text-white sm:p-8 lg:p-10">
                  <div className="flex items-start justify-between gap-5"><div><p className="text-[0.62rem] font-semibold uppercase tracking-[0.18em] text-[#E6C767]">{tx("At your horizon")}</p><h3 className="mt-3 font-display text-3xl font-medium tracking-[-0.05em]">{formatCurrency(calculation.holdingProjection.netWorth, lang)}</h3></div><ArrowUpRight className="h-6 w-6 text-[#E6C767]" strokeWidth={1.2} aria-hidden="true" /></div>
                  <div className="mt-8 grid gap-0 sm:grid-cols-3 sm:gap-5">
                    <div className="border-t border-white/20 pt-3"><p className="text-[0.58rem] uppercase tracking-[0.16em] text-white/55">{tx("Property value")}</p><p className="mt-2 text-sm font-semibold">{formatCurrency(calculation.holdingProjection.propertyValue, lang)}</p></div>
                    <div className="border-t border-white/20 pt-3"><p className="text-[0.58rem] uppercase tracking-[0.16em] text-white/55">{tx("Remaining debt")}</p><p className="mt-2 text-sm font-semibold">{formatCurrency(calculation.holdingProjection.remainingDebt, lang)}</p></div>
        ...
        <section className="border-b border-[#161616]/12 bg-[#F3EDE1]">
          <div className="mx-auto w-full max-w-[1600px] px-5 py-14 sm:px-8 sm:py-20 lg:px-14 lg:py-28">
            <div className="grid gap-12 lg:grid-cols-[0.72fr_1.28fr] lg:gap-20">
              <div>
                <p className="brandbook-eyebrow">04 — {tx("Projection")}</p>
              ...
              <div>
                <div className="grid gap-px bg-[#161616]/12 sm:grid-cols-2 lg:grid-cols-3">
                  {calculation.milestones.map((milestone) => (
                    <article key={milestone.year} className="bg-[#FAF8F3] p-5 sm:p-6">
                      ...
                      <p className="mt-6 font-display text-2xl font-medium tracking-[-0.045em]">{formatCurrency(milestone.netWorth, lang)}</p>
                ...
                <div className="mt-5 border-t border-[#161616]/15 pt-5">
                  <div className="grid gap-0 sm:grid-cols-3 sm:gap-5">
                    <DetailRow label={tx("Property value")} value={formatCurrency(calculation.holdingProjection.propertyValue, lang)} />
                    <DetailRow label={tx("Remaining debt")} value={formatCurrency(calculation.holdingProjection.remainingDebt, lang)} />
                    <DetailRow label={tx("Accumulated cash flow")} value={formatCurrency(calculation.holdingProjection.accumulatedCash, lang)} />
                  </div>
                </div>
              </div>
            </div>
        ...
        <section className="bg-[#002147] text-white">
          <div className="mx-auto grid w-full max-w-[1600px] gap-12 px-5 py-14 sm:px-8 sm:py-20 lg:grid-cols-[0.78fr_1.22fr] lg:gap-20 lg:px-14 lg:py-24">
            <div>
              <p className="brandbook-eyebrow brandbook-eyebrow-light">05 — {tx("Download investment brief")}</p>
              <h2 className="mt-7 max-w-[9ch] font-display text-[clamp(3rem,5.5vw,6rem)] font-medium leading-[0.87] tracking-[-0.07em]">{tx("Download localized PDF brief")}</h2>
              <p className="mt-8 max-w-[25rem] text-base leading-[1.6] text-white/60">{tx("The figures are illustrative and depend on unit selection, financing, occupancy, market conditions and delivery.")}</p>
              <Link href="/reverance-batumi" className="mt-10 inline-flex items-center gap-3 text-[0.68rem] font-semibold uppercase tracking-[0.18em] text-[#E6C767] transition-colors hover:text-white">{tx("Back to Reverance")} <MoveRight size={17} strokeWidth={1.5} /></Link>
            </div>
            <div className="border border-white/18 bg-white/[0.035] p-6 sm:p-8 lg:p-10">
              <div className="flex items-start justify-between gap-5 border-b border-white/15 pb-6"><div><p className="text-[0.62rem] font-semibold uppercase tracking-[0.18em] text-[#E6C767]">{tx("Selected language")}</p><p className="mt-3 text-2xl font-medium tracking-[-0.04em]">{lang.toUpperCase()}</p></div><ShieldCheck className="h-7 w-7 text-[#E6C767]" strokeWidth={1.2} aria-hidden="true" /></div>
              <label className="mt-8 grid gap-3"><span className="text-[0.65rem] font-semibold uppercase tracking-[0.16em] text-white/58">{tx("Your name (optional)")}</span><input value={clientName} onChange={(event) => setClientName(event.target.value)} placeholder={tx("Used for the cover only")} maxLength={100} className="min-h-13 w-full border border-white/20 bg-white/[0.06] px-4 text-base text-white outline-none placeholder:text-white/35 focus:border-[#E6C767] focus:ring-2 focus:ring-[#E6C767]/30" /></label>
              <button type="button" onClick={downloadPdf} disabled={pdfState === "loading"} className="mt-7 inline-flex min-h-14 w-full items-center justify-center gap-3 bg-[#E6C767] px-5 text-[0.68rem] font-semibold uppercase tracking-[0.17em] text-[#161616] transition-colors hover:bg-white disabled:cursor-wait disabled:opacity-60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#E6C767] focus-visible:ring-offset-2 focus-visible:ring-offset-[#002147]">
                <Download size={16} strokeWidth={1.8} aria-hidden="true" /> {pdfState === "loading" ? tx("Generating PDF…") : tx("Download localized PDF brief")}
              </button>
              <p className="mt-4 min-h-6 text-sm text-white/65" aria-live="polite">{pdfState === "success" ? tx("PDF brief downloaded.") : pdfState === "error" ? tx("Could not generate the PDF. Please try again.") : ""}</p>
              <div className="mt-7 flex items-start gap-3 border-t border-white/15 pt-5 text-xs leading-5 text-white/52"><ArrowDownRight className="mt-0.5 h-4 w-4 shrink-0 text-[#E6C767]" strokeWidth={1.3} /><span>{tx("This is not financial, legal or tax advice.")}</span></div>
            </div>
          </div>
        </section>
      </main>
