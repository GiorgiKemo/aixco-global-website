...
import { adminInviteEmailSchema } from "./identity-migration";
...
function buildInviteText(email: string, actionLink: string) {
  return [
    ...
    "",
    "Open this secure invitation link to create your password and enroll MFA:",
    actionLink,
    "",
  ...
  <body style="margin:0;background:#f3ede1;color:#161616;font-family:Arial,Helvetica,sans-serif;">
    <div style="max-width:640px;margin:0 auto;padding:32px 18px;">
      <div style="background:#ffffff;border-top:5px solid #9c7f3c;padding:36px 34px;">
        ...
        <p style="margin:24px 0 0;color:#6f6e6a;font-size:13px;line-height:1.6;">This secure link is single-use. It will take you to AIXCO to create a password and enroll your authenticator. If it expires, an administrator can send another invitation from the dashboard.</p>
...
export async function resendAdminIdentityInvite(
  ...
  if (generated.error || !generatedUser || generatedUser.id !== options.expectedUserId || !actionLink) {
  ...
  if (actionUrl.searchParams.get("redirect_to") !== options.redirectTo) {
  ...
  const response = await (options.fetchImpl ?? fetch)("https://api.resend.com/emails", {
    ...
    headers: {
      ...
      "Idempotency-Key": `admin-invite-resend-${options.expectedUserId}-${randomUUID()}`,
    },
    body: JSON.stringify({
      from: config.from,
      to: [normalizedEmail],
      ...
      tags: [{ name: "source", value: "admin_identity_invite_resend" }],
    }),
  });
  ...
  if (!response.ok) {
    throw new Error("The administrator invitation email could not be sent.");
  }
