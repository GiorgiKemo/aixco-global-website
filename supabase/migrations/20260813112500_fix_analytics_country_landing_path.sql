...
begin
  if p_start is null or p_end is null or p_start >= p_end then
    raise exception 'A valid analytics window is required.' using errcode = '22023';
  end if;
      ...
      select
        ...
        count(*) filter (
          where lower(coalesce(sessions.referrer_host, '')) in ('localhost', '127.0.0.1', '::1')
             or sessions.landing_path like '%codex-production-smoke%'
...
comment on function public.get_site_analytics_country_breakdown(
  ...
  timestamptz,
  integer
) is 'Returns bounded country-level visitor and engagement quality for the service-role admin dashboard.';
